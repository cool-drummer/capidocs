...
describe('portal, space and pages', () => {
  ...
  it('creates a portal', async () => {
    const response = await call('POST', '/portals', {
      name: 'Acme',
      slug: 'acme',
      siteUrl: 'https://acme.example/docs',
      locales: ['es'],
      accessMode: 'open',
  ...
  it('creates a hero page and two content pages', async () => {
    const home = await call('POST', `/spaces/${ids.space}/pages`, { title: 'Documentación de Acme', kind: 'hero' });
  ...
  it('returns the tree in position order', async () => {
    ...
    expect(titles).toEqual(['Documentación de Acme', 'Primeros pasos', 'Seguridad']);
  });
  ...
  it('writes a runnable site when the portal has a directory target', async () => {
    const portal = await call('POST', '/portals', {
      name: 'Globex',
      slug: 'globex',
      siteUrl: 'https://globex.example/docs',
      locales: ['es'],
    ...
    const page = await call('POST', `/spaces/${space.body.space.id}/pages`, { title: 'Inicio Globex', kind: 'hero' });
    ...
    expect(readFileSync(join(outDir, 'robots.txt'), 'utf8')).toContain('https://globex.example/docs/sitemap.xml');
  });
});
